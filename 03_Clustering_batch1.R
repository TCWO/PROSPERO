...
write.csv2(x = tmp_reduced, file = file.path('..', 'Data', 'CellLines', 'Clustering', 'df_data_clusters_batch1.csv'), row.names = FALSE)
...
for(i in unique(df_heatmap$clustering_method)){
  ...
  tmp_plot <- tmp_plot %>%
    ...
    arrange(-Rank) %>%
    dplyr::select(-Rank)
  write.csv2(x = tmp_plot, file = file.path('..', 'Data', 'CellLines', 'Clustering', 'Clusters_per_method', paste0(i, '_clusters_sorted_batch1.csv')), row.names = FALSE)
}